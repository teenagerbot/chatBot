class ChatBot {
	newMemory() {
		...
		for (c; c < user.length; c++) {
			let question = user[c].match(/(\|([a-zA-Z0-9\sа-яА-Я,\.%@#=$&\-+_:;'"*(){}<>\[\]~∆÷×π√•^°¥€¢£✓™®©‰℅]+))/gim) + ":";
			questions.push(question);
		} 
		for (a; a < user.length; a++) {
			let answer = user[a].replace(/(\|([a-zA-Z0-9\sа-яА-Я,\.%@#=$&\-+_:;'"*(){}<>\[\]~∆÷×π√•^°¥€¢£✓™®©‰℅]+))/gim, "") + "|";
			answers.push(answer);
		}
  ...
  startBot() {
    ...
    botMessage.style.cssText = `border: 2px blue solid; border-radius:20px;width: fit-content;padding:6px; background:lightblue;`;
		let helloMessage = (lang === "ru")?"Привет, чем могу помочь?":(lang === "ua")?"Привіт, чим можу тобі допомогти?":(lang === "en")?"Hello, how can I help?":(lang === "de")?"Hallo, wie kann ich helfen?":(lang === "es")?"Hola, ¿cómo puedo ayudar?":(lang === "fr")?"Bonjour, comment puis-je vous aider ?":(lang === "it")?"Ciao, come posso aiutarti?":(lang === "ja")?"こんにちは、どうすれば手伝うことができますか？":(lang === "ka")?"გამარჯობა, როგორ შემიძლია დაგეხმაროთ?":(lang === "pl")?"Witam w czym mogę pomóc?":(lang === "sr")?"Здраво, како могу да помогнем?":(lang === "zh")?"您好，請問有什麼可以幫忙的嗎？":"Hello, how can I help?";
		let call = (lang === "ru")?"Не понял, ещё раз скажите пожалуйста внятнее 🤔":(lang === "ua")?"Не зрозумів, ще раз скажіть будь ласка виразніше 🤔":(lang === "en")?"I don't understand, please tell me more clearly 🤔":(lang === "de")?"Ich verstehe nicht, bitte sagen Sie es mir deutlicher 🤔":(lang === "es")?"No entiendo, por favor dímelo más claro 🤔":(lang === "fr")?"Je ne comprends pas, s'il vous plaît dites-moi plus clairement 🤔":(lang === "it")?"Non capisco, per favore dimmelo più chiaramente 🤔":(lang === "ja")?"わかりません、もっとはっきり教えてください 🤔":(lang === "ka")?"არ მესმის, გთხოვ უფრო გარკვევით მითხარი 🤔":(lang === "pl")?"Nie rozumiem, proszę powiedz mi wyraźniej 🤔":(lang === "sr")?"Не разумем, молим те реци ми јасније 🤔":(lang === "zh")?"沒看懂，說清楚點 🤔":"I don't understand, please tell me more clearly 🤔";
		if (time === "no_time") {
			no_time();
		} else {
			yes_time();
          ...
          us_msg.style.cssText = `border: 2px green solid; border-radius:20px;width: fit-content;padding:6px; display:block; position:relative;margin-left:auto; background:lightblue;`;
              ...
              setTimeout(function() {
                ...
                document.querySelector("#___window_chat_bot___").appendChild(p); 
							...
							let err = document.createElement("p");
              err.innerHTML = "¯\\_(ツ)_/¯" + ` <tm style='font-size:10px;'>${hate.getHours()}:${hate.getMinutes()}</tm>`;
              err.style.cssText = `border: 2px blue solid; border-radius:20px;width: fit-content;padding:6px;background:lightblue;`;
							let type = document.createElement("img");
              type.src = "load.gif";
              type.style.cssText = `width:50px;`;
